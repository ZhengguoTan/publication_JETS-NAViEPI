...
\begin{document}
    ...
    \begin{frontmatter}
        ...
        \begin{abstract}
            ...
            \marginnote{R248.Major.6}
            we developed \hl{a} $k_y$-shift encoding
            ...
            \hl{Our method was demonstrated with experimental results
    ...
    of single-shot EPI acquisition \hl{and} \marginnote{R248.Major.2}
    ...
    \begin{threeparttable}
        ...
        \begin{tabular}{ a | b b b b }
            ...
            FOV (\si{\square\mm}) & $200$ & \multicolumn{3}{c}{$220$} \\
    ...
    \begin{enumerate}[I.]
        ...
        of a spatial-diffusion matrix.} \marginnote{R249.Minor.9}
        ...
        % This work employed blipped-CAIPI SMS
        % \citep{setsompop_2012_blipped},
        ...
        % \marginnote{R249.1-3}
    	% \hl{We overcame this problem by utilizing overlapping blocks.
    	% If the blocks overlap,
        % $\mathbf{T}^H (\mathbf{T} (\mathbf{x})) \neq \mathbf{x}$.
    	% $\mathbf{T}^H$, the Hermitian adjoint operator of $\mathbf{T}$,
        % is used after the SVT operation to
        % rearrange the spatial-diffusion matrices back into
        % their original positions in DW images.
    	% During this procedure, values from overlapping positions
        % are summed in the output of $\mathbf{T}^H$.
    	% Our solution was to element-wise divide
        % the output of $\mathbf{T}^H$
        % by a scaling matrix. This matrix can be obtained via
        % $\mathbf{T}^H (\mathbf{T}(\mathbf{1}))$, where
    	% $\mathbf{1}$ denotes the matrix of all ones
    	% with the same shape as the input $\mathbf{x}$.}
    \end{enumerate}
    ...
    \marginnote{R249.Minor.21}
    with the use of \hl{a} 5-shot acquisition.
    \marginnote{R249.Minor.22}
