...
\begin{document}
    ...
    \begin{frontmatter}
        ...
        \begin{abstract}
            ...
            which enforces consistent bandwidth
            ...
            \marginnote{R1.6}
            \hl{we develop the \mbox{$k_y$}-shift encoding
            among diffusion encodings
            to explore complementary \mbox{$k$}-\mbox{$q$}-space sampling.
            Moreover, we develop a novel joint reconstruction
            with overlapping locally low-rank regularization}
            generalized to multi-band multi-shot acquisition
    ...
    % Its \hl{properties} are two-fold. \marginnote{R1.21}
    ...
    \begin{threeparttable}
        ...
        \begin{tabular}{ a | b b b b }
            ...
            \rowcolor{gray!20} Diffusion direction & \multicolumn{2}{c}{$20$} & \multicolumn{2}{c}{$3$} \\
            $b$-value (\si{s/mm^2}) & \multicolumn{4}{c}{1000} \\
            \rowcolor{gray!20} Voxel size (\si{\cubic\mm}) & \multicolumn{2}{c}{$1.0$} & \multicolumn{2}{c}{$0.5\times0.5\times2.0$} \\
            Slices & \multicolumn{2}{c}{$141$} & \multicolumn{2}{c}{$60$} \\
            \rowcolor{gray!20} FOV (\si{\square\mm}) & \multicolumn{2}{c}{$200$} & \multicolumn{2}{c}{$220$} \\
            Base resolution & \multicolumn{2}{c}{$200$} & \multicolumn{2}{c}{$440$} \\
            \rowcolor{gray!20} Navigator & No & Yes & Yes & No \\
            Shots & $4$ & $3$ & $5$ & $1$ \\
            \rowcolor{gray!20} TR (\si{ms}) & $7700$ & $7600$ & $4400$ & $8000$ \\
            TEs (\si{ms}) & $67$ & $58/78$ & $58/95.1$ & $143$ \\
            \rowcolor{gray!20} Echo spacing (\si{ms}) & $1.02$ & $1.08$ & $1.52$ & $1.48$ \\
            Bandwidth (\si{Hz/Pixel}) & \multicolumn{2}{c}{$1086$} & \multicolumn{2}{c}{$758$} \\
            \rowcolor{gray!20} Partial Fourier & \multicolumn{4}{c}{$6/8$} \\
            Acceleration~\tnote{2} & $1\times3$ & $3\times3$ & \multicolumn{2}{c}{$3\times2$} \\
            \rowcolor{gray!20} TA (\si{\minute})~\tnote{3} & $10:42$ & $8:02$ & $1:38$ & $0:46$ \\
            \bottomrule
        \end{tabular}
    ...
    % To demonstrate the acquisition efficiency of NAViEPI,
    % a sub-millimeter protocol was implemented.
    % This protocol employed \SI{220}{\milli\meter} FOV
    % in both read and phase-encoding directions,
    % base resolution 440, 60 slices, bandwidth \SI{758}{Hz/Pixel},
    % echo spacing \SI{1.52}{ms}, TEs \SI{$58/95.1$}{ms}, TR \SI{4400}{ms}.
    % Five shots per diffusion encoding with partial Fourier $6/8$
    % along the phase-encoding direction was used.
...
%    as \hl{in} \cref{SEC:ACQ1.2mm}. \marginnote{R1.22}
...
%    This protocol \hl{was used to} demonstrate \marginnote{R1.23}
...
%    spatial-angular-temporal resolution.
    ...
    %     the fODF peaks show improved \hl{smoothness}, \marginnote{R1.44}
